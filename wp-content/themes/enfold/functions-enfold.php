...
if( ! function_exists( 'avia_header_setting' ) )
{
	...
	function avia_header_setting( $single_val = false )
	{
		...
		/**
		 * Select to display on pages which are not singular (e.g. shop page in WooCommerce)
		 *
		 * @used by					enfold\config-woocommerce\config.php							10
		 * @since 5.6
		 * @param boolean $force_posts_layout_settings
		 * @return boolean									return true to use
		 */
		$force_posts_layout_settings = apply_filters( 'avf_force_posts_layout_settings', false, $post_id );

		if( $post_id && ( is_singular() || true === $force_posts_layout_settings ) )
		{
