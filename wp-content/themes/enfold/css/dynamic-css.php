...
";
	...
	{
		$section_key = str_replace( '_', '-', $section_key );
		...
		foreach( $colors as $color_key => $color )
		{
			...
			$output .= "--enfold-{$section_key}-{$color_key}: {$color};\n";
		}
	}
...
foreach( $color_set as $key => $colors ) // iterates over the color sets: usually $key is either: header_color, main_color, footer_color, socket_color
{
	...
	$bg_rgb = avia_backend_hex_to_rgb_array( $bg );
