...
{
	class avia_adminpages extends aviaFramework\base\object_properties
	{
		/**
		...
		public function add_scripts()
		{
			...
			foreach ( $files as $index => $file )
			{
				...
				if( isset( $file_info['extension'] ) && $file_info['extension'] == 'js' )
				{
					...
					wp_enqueue_script(  $filename, AVIA_JS_URL . $file , false, $vn );
				}
			}
		}
		...
		public function add_styles()
		{
			...
			foreach( $files as $index => $file )
			{
				...
				if( isset( $file_info['extension'] ) && $file_info['extension'] == 'css' )
				{
					...
					wp_enqueue_style( $filename, AVIA_CSS_URL . $file, false, $vn );
				}
			}
		...
		*/
		protected function add_global_styles()
		{
			...
			wp_enqueue_style( 'avia_global_admin', AVIA_CSS_URL . "conditional_load/avia_global_admin{$min_css}.css",  false, $vn );
		}
		...
		*/
		public function attach_options_to_menu()
		{
			...
			{
				/**
				 * if its the very first option item make it a main menu with theme or plugin name,
				 * then as first submenu add it again with real menu name
				 */
				if( empty( $top_level ) )
				{
					...
					$avia_page = add_menu_page(			$the_title, 									// page title
