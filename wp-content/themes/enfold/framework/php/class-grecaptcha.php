...
if( ! class_exists( 'av_google_recaptcha' ) )
{
	class av_google_recaptcha extends aviaFramework\base\object_properties
	{
		...
		public function handler_wp_enqueue_scripts()
		{
			...
			//	@since 5.6.1 https://kriesi.at/support/topic/ubersetzungsfehler-translation-mistake/#post-1404182
			$local = get_locale();
			$api_url_lang = ( empty( $local ) ) ? av_google_recaptcha::API_URL_LANG : substr( $local, 0, 2 );

			/**
			 * @since 5.3
			 * @since 5.6.1						added $default
			 * @param string $api_url_lang
			 * @param string $context			'backend' | 'frontend'
			 * @param string $default
			 * @return string
			 */
			$api_url_lang = apply_filters( 'avf_google_recaptcha_apiurl_lang', $api_url_lang, 'frontend', av_google_recaptcha::API_URL_LANG );
...
//					'submission_nonce'	=> wp_create_nonce( av_google_recaptcha::AJAX_SUBMISSION_NONCE ),
		...
		public function handler_wp_admin_enqueue_scripts()
		{
			 ...
			 * @return string
			 */
			$api_url_lang = apply_filters( 'avf_google_recaptcha_apiurl_lang', $api_url_lang, 'backend', av_google_recaptcha::API_URL_LANG );
