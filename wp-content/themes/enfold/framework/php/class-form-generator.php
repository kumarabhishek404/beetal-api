...
if( ! class_exists( 'avia_form' ) )
{
	class avia_form extends aviaFramework\base\object_properties
	{
		...
		/**
		 * Local value from static variable $form_id
		 *
		 * @var int
		 */
		protected $formID;
		...
		/**
		 * This string holds the final html output
		 *
		 * @var string
		 */
		var $output;
		...
		/**
		 * Error message that is shown in the response area after submitting with ajax
		 *
		 * @since 4.5.7.2
		 * @var string
		 */
		...
		/**
		 * Success message that is shown in the response area after submitting with ajax and overrrides default success message
		 *
		 * @since 5.4.1
		 */
		...
		public function __construct( $params )
		{
			...
			$this->submit_attr = '';
			$this->do_checks = true;
			$this->autoresponder = array();
			$this->length = 20;
			$this->width = 1;
			$this->width_translate = array(
										'fullwidth'				=> 1,
										'element_half'			=> 0.5,
										'element_fourth'		=> 0.25,
										'element_third'			=> 0.3,
										'element_two_third'		=> 0.6,
										'element_three_fourth'	=> 0.75
									);
			...
			$form_data .= 'data-fields-with-error="' . __( 'Found errors in the following field(s):', 'avia_framework' ) . '"';
		...
		public function display_form( $return = false )
		{
			...
			if( $this->submit_form )
			{
				...
				else
				{
					...
					else if( ! empty( $this->submit_message ) )
					{
						$success .= $this->submit_message;
					}
					else
					{
