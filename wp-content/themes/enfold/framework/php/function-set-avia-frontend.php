...
//		$use_image = apply_filters( 'avf_logo', $use_image );	//	since 4.5.7.2 changed as inconsistently used again when logo is set
	...
	//					$dimension = '';
				...
				else
				{
					...
					$logo_img = avia_SVG()->get_html( $logo_id, $logo_url, avia_SVG()->get_header_logo_aspect_ratio() );
				}
				...
				$logo = "<{$headline_type} class='{$logo_class}'><a href='{$link}' class='{$a_class}'>{$logo_img}{$sub}</a></{$headline_type}>";
			}
		}
		else
		{
