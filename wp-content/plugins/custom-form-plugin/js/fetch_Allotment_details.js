...
jQuery(document).ready(async function ($) {
  jQuery("#form2-fetch").on("submit", async function (e) {
    ...
    console.log(option_value, option_value, company_isin);
    ...
    if (!company_isin || !login_option || !option_value) {
      jQuery("#error-form2-result").html(`
                        <div id="form-message" style="color: red; margin-top: 10px;">
      ...
      await grecaptcha
        ...
        .then(function (token) {
          ...
          jQuery.ajax({
            ...
            type: "POST",
            data: {
              action: "fetch_entitlement_details",
              company_isin,
              login_option,
              option_value,
              token,
            ...
            success: function (response) {
              if (response.success) {
                ...
                if (response.data?.total > 0) {
                  jQuery("#form2-result").html(
                    offer_html(response?.data?.data)
                  );
  ...
  jQuery("#ipo-allotment-form").on("submit", async function (e) {
    e.preventDefault();
    jQuery("#form-message").remove();
          ...
          jQuery.ajax({
            ...
            success: function (response) {
              if (response.success) {
                ...
                if (response.data?.total > 0) {
                  jQuery("#allotment_result").html(
                    ipo_allotment_html(response.data?.data)
                  );
                  jQuery('[data-toggle="tooltip"]').tooltip();
                } else {
                  jQuery("#allotment_result").html(`
  ...
  $("input[type='radio'][name='form2_radio_option_otp']").change(function () {
    let selectedValue = $("input[name='form2_radio_option_otp']:checked").val();
