...
{
    wp_enqueue_style('short-codes-css', plugin_dir_url(__FILE__) . 'css/styles.css');
...
{
    wp_enqueue_script('dynamic-field-script', plugin_dir_url(__FILE__) . 'js/script.js', array('jquery'), '1.0.2', true);
    wp_localize_script('dynamic-field-script', 'ajaxurl', admin_url('admin-ajax.php'));
    wp_enqueue_script('validate-script', plugin_dir_url(__FILE__) . '/js/jquery.validate.js', array(), '1.0.2', true);
}
...
function custom_user_login()
{
    ...
    if (username_exists($folio_number)) {
        ...
        wp_send_json_success(['message' => 'You’re in! We’re redirecting you to your folio page. Please wait a moment.', "redirect_url" => site_url('investor-service-request')]);
    }
    ...
    if (is_wp_error($user_id)) {
        ...
        wp_send_json_success(['message' => "User ‘{$folio_number}’ has been created successfully! Redirecting you to your folio page…", "redirect_url" => site_url('investor-service-request')]);
    }
}
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($username) || empty($password)) {
            ...
            if ($user && wp_check_password($password, $user->user_pass)) {
                ...
                $user_role = in_array('wpas_company_user', $user->roles);
...
function send_otp()
{
    ...
    if ($type === 'email') {
        ...
        $message = "Your OTP for Investor Service Request is $otp. Please use this code to complete your verification. Do not share it with anyone.";
        $headers = ['Content-Type: text/html; charset=UTF-8'];

        if (wp_mail($recipient, $subject, $message, $headers)) {
            wp_send_json_success('We’ve sent the OTP to your email. Kindly check your inbox and spam folder.');
        } else {
        ...
        wp_send_json_success("We’ve sent the OTP to your phone: $recipient. Check your SMS for the code.");
    } else {
...
function fetch_ipo_allotment_details_fn()
{
    ...
    if ($login_option === 'Enter Application Number.') {
        $login_option = 'application_number';
    } else if ($login_option === 'Enter PAN Number.') {
        $login_option = 'pan_number';
    } else if ($login_option === 'Eg:-INXXXXXXXXXX,120XXXXXXXXX,XXXXXXX') {
        $login_option = 'client_id';
    }

    $pods = pods('ipo_allotment_data')->find(array(
        'where' => array(
    ...
    if ($pods->total() > 0) {
        wp_send_json_success($pods->fetch());
    } else {
...
?>
        <script>
            window.location.href = `<?= site_url('investor-service-request/?tab=company_information');?>`;
            </script>
            <?php
        }
    }
